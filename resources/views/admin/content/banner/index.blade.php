...
@section('content')
    <div class="content-header text-right">
        <div class="container-fluid">
            <div class="row mb-4">
                <div class="col-sm-6">
                    <a role="button" href="{{ route('admin.content.banner.create') }}" type="button"
                       class="btn btn-primary btn-sm">
                        <i class="fa fa-plus-circle"></i> ایجاد بنر جدید
                    </a>
                </div><!-- /.col -->
                <div class="col-sm-6">
            ...
            <div class="row border-1 pb-2 pt-2 pr-1 pl-1 mr-2 d-flex align-items-center"
                ...
                <strong class="text-muted mr-1 ml-1">
                    <small>
                        در این قسمت می توانید بنرهای بخش فروشگاه سایت خود را ایجاد و یا مدیریت کنید.
                        <br><strong class="text-success mt-2" style="margin-right: auto">تعداد کل بنر ها
                            : {{ $banners->count() != 0 ? $banners->count() : 'صفر' }}</strong>
                    </small>
                </strong>
            </div>
        </div><!-- /.container-fluid -->
                            ...
                            <table id="example"
                                ...
                                @foreach($banners as $key => $banner)
                                    <tr role="row" class="odd">
                                        ...
                                        <td>{{ $banner->title }}</td>
                                        <td>{{ $banner->slug }}</td>
                                        <td title="{{ $banner->description ?? $banner->title }}">{{ Str::limit($banner->description,30) ?? $banner->title }}</td>
                                        ...
                                        <td>{{ persianDateTime($banner->created_at) }}</td>
                                        <td class="">
                                            <label for="status">
                                                <input
