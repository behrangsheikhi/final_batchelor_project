...
@section('head-tag')
    <title>محتوا | نظرات | مشاهده نظر</title>
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-12">
                    <ol class="breadcrumb float-right mb-4">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('admin.content.comment.index') }}">نظرات</a>
                        </li>
            ...
            <div class="row border-1 pb-2 pt-2 pr-1 pl-1 mr-2 d-flex align-items-center"
                ...
                <strong class="text-muted mr-1 ml-1">
                    <small>
                        در این قسمت می توانید نظر انتخاب شده را مشاهده و مدیریت کنید..
                    </small>
                </strong>
            </div>
        </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <section class="card mb-3">
                            ...
                            <section class="card-body">
                                <div class="row mb-4 justify-content-between">
                                    <span class="d-flex">
                                       <p class="text-primary text-sm mr-3"> عنوان پست : </p>
                                        <p class="font-weight-bold">{{ $comment->commentable->title ?? '-' }}</p>
                                    </span>
                        ...
                        <section
                            ...
                            تاریخچه نظر های کاربر به این پست:
                        </section>
                        <div class="box direct-chat direct-chat-info" id="box">
                            <div class="box-header with-border">
                                <h4 class="box-title">{{ $comment->commentable->title ?? '-' }}</h4>
                                    ...
                                    <div class="direct-chat-messages"
                                        ...
                                        <div class="direct-chat-msg mb-30">
                                            ...
                                            <div class="direct-chat-text">
                                                ...
                                                <p class="direct-chat-timestamp">
                                                    <time datetime="2018">{{ persianDateTime($comment->created_at) }}</time>
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        @if($comment->children)
                                            @foreach($comment->children as $reply)
                                                @include('admin.content.comment._reply_partial')
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
                        ...
                        <section class="comment-reply-section" id="comment-reply-section">
                            @include('admin.content.comment.market_comment_partial',['comment' => $comment])
                            @yield('comment')
                        </section>
                    </div>
                </div>
...
<script type="text/javascript">
    function addComment(event) {
        ...
        let url = "{{ route('admin.content.comment.reply-to-comment',$comment->id ?? "") }}";
        $.ajax({
            url: url,
            data: formData,
            dataType: "json",
