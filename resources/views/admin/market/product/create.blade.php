                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <section class="col-12 border-bottom py-4 my-4">
                            ...
                            <section>
                                <button type="button" id="btn-copy" class="btn btn-xs btn-success mb-4">افزودن</button>
                            </section>
                        </section>
                    </div>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            let related_products_input = $('#related_products');
            let select_related_products = $('#select_related_products');
            let default_related_products = related_products_input.val();
            let default_data = null;


            if (related_products_input.val() !== null && related_products_input.val().length > 0) {
                default_data = default_related_products.split(',');
            }

            select_tags.select2({
                placeholder: 'لطفا کالاهای مرتبط این کالا را وارد نمایید',
                tags: true,
                data: default_data,
                dir: "rtl"
            });
            select_related_products.children('option').attr('selected', true).trigger('change');


            $('#form').submit(function (event) {
                if (select_related_products.val() !== null && select_related_products.val().length > 0) {
                    let selectedSource = select_related_products.val().join(',');
                    related_products_input.val(selectedSource);
                }
            })
        });
