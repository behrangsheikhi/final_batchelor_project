...
@section('content')
    <section class="vh-100 d-flex justify-content-center align-items-center pb-5">
        <form action="{{ route('vendor.auth') }}" method="post">
            @csrf
            <section class="login-wrapper mb-5">
                <section class="login-logo">
                    <img src="{{ asset('assets/images/logo/logo.jpeg') }}" alt="logo">
{{--                    <h1 class="text-right font-weight-bold login-title" style="font-weight: 700">ورود | ثبت نام</h1>--}}
                </section>
                <section class="login-title">
                    <small class="login-info text-muted">جهت ادامه شماره موبایل یا ایمیل خود را وارد کنید.</small>
                </section>
                <section class="login-input-text">
                    {{-- identity can be email or mobile --}}
                    <input type="text" name="identity" value="{{ old('identity') }}" class="outline text-center border-1"
                           autofocus
                ...
                <section class="login-btn d-grid g-2">
                    <button type="submit" class="btn font-weight-bold text-white custom-btn"
                            onclick="otpCodeSent()"
                            data-url="{{ route('vendor.auth') }}">ورود</button>
                </section>
